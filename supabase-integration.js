// ============================================================
//  supabase-integration.js — v3 FINAL
//  Kanban Reforma Tributária · TOTVS
//  - Autenticação por e-mail e senha
//  - Gerente vê todos os projetos
...
function atualizarTopbar() {
  ...
  `;
  // Consultor: oculta botões de outros consultores
...
function iniciarAutoRefresh() {
  ...
  _autoRefreshTimer = setInterval(async () => {
    // Não atualiza se há modal aberto (evita perda de dados não salvos)
    const modalAberto = document.querySelector('.modal-overlay.open, .modal-overlay[style*="flex"]');
    if (modalAberto) return;
    console.log('🔄 Auto-refresh...');
    await carregarDadosDoSupabase(true); // true = silencioso (sem overlay)
  }, 60000); // 60 segundos
}

// Botão de atualizar manual na topbar
...
async function carregarDadosDoSupabase(silencioso = false) {
  ...
  try {
    ...
    // Gerente vê tudo, consultor vê apenas seus projetos
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if(sessao){
      ...
      if(res.ok){ await iniciarApp(); adicionarBotaoRefresh(); return; }
    }
  } catch(e){}
